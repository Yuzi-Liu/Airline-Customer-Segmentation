# This program contains the R code for customer segmentation for the Sun Country Airlines case
...
# normalize the data before doing our segmentation analysis.
normalize <- function(x){
  return ((x - min(x))/(max(x) - min(x)))
}

sg_data = mutate(sg_data,
                    amt = normalize(amt),
...
sg_data <- as.data.frame(sg_data)
sg_data <-  sg_data %>% 
  cbind(acm.disjonctif(sg_data[,c("BookingChannel","age_group",
                               "true_origin","true_destination","UflyMemberStatus","seasonality")]))%>% 
  ungroup()

# Remove the original (non-dummy-coded) variables
sg_data<-sg_data %>%select(-BookingChannel,-age_group,-true_origin,-true_destination,-UflyMemberStatus,-seasonality)

# look at the within SSE curve; 3 - 5 seems like the best solution here

SSE_curve <- c()
for (n in 1:15) {
  kcluster <- kmeans(sg_data, n)
